module.exports = 
{
	dan: function(arg, msg, Danbooru, sfwEmote, nsfwEmote, questionEmote, safeStatus, adminRole)
	{
		...
		else
		{
			...
			console.log('Of type: ' + typeof spaceCount);
			console.log('-----');
			if(spaceCount > 1)
			{
				msg.channel.send("Sorry, Danbooru doesn't let us search more than 2 tags at once ;-;");
				return;
			};
		};
			...
			booru.posts({ tags: arg}).then(posts => 
			{
				...
				try
				{
					...
					require('https').get(url, response => 
					{
						...
						{
							console.log('Video file detected, notifying user...');
							console.log('-----');
	...
	gel: function(arg, msg, sfwEmote, nsfwEmote, questionEmote, request, parseString, jsdom, { JSDOM }, { window }, { document }, $, fs, readline, safeStatus, adminRole) 
	{
		console.log('Command recieved.');
		...
		{
			if (safeStatus == 'true' && !msg.member.roles.find("name", adminRole))
			{
	...
	lewd: function(arg, msg, sfwEmote, nsfwEmote, questionEmote, request, parseString, jsdom, { JSDOM }, { window }, { document }, $, fs, readline, safeStatus, adminRole, total)
	{
		console.log('Command recieved.');
		console.log('-----');
