module.exports = 
{
	...
	prefs: function(fs, arg, args, msg, adminRole, lewdNum, superSafeMode, currentDir, client)
	{
		if(msg.member.roles.find("name", adminRole))
		{
			if (arg.includes('lewdupdate '))
			{
				...
				msg.channel.send("Restarting for update to take effect...");
				client.users.get("322041078385737739").send(sender + ' updated the number of images in lewdbooru from: ' + msg.channel.name);
				console.log('-----');
			}
			else if (arg.includes('safetoggle '))
			{
				console.log('Authority confirmed.');
				var tf = arg.substr(arg.indexOf(' ')+1);
				var tf = tf.substr(0,args.indexOf(' '));
				var sender = msg.author.username + '#' + msg.author.discriminator;
				console.log('To file: ' + tf);
				fs.writeFile(currentDir + '/preferences/safeStatus.txt', tf, function (err) 
				{
					if (err) throw err;
					console.log('Saved!');
				});
				msg.channel.send("Restarting for update to take effect...");
				client.users.get("322041078385737739").send(sender + ' changed the safe mode status from: ' + msg.channel.name);
				console.log('-----');
			};
		}
		else
	...
	reboot: function(arg, msg, adminRole, token, client)
	{
		if(msg.member.roles.find("name", adminRole))
		{
			try
			{
				...
				console.log('Doing the time warp again...');
				console.log('-----')
	...
	changelog: function (arg, msg) 
	{
        ...
        {
			color: 0xff5511,
			title: "Megumeme bot changelog as of version 2.4:",
			description: "A semi-detailed list of all recent changes. Only covers publically available builds.",
			fields: 
			[	
				{
					name: "2.4",
					value: "THE INTERNAL COMBUSTION UPDATE: \nFixed Lewdbooru command after site's ad integration. \nBot now appropriately responds to DMs. \nCleaned up all the leftovers and mistakes across the board. \nMade the pokémon commands far more streamlined."
				},
				{
					name: "2.3",
