	...
	spamCheck: function(arg, msg, msgHistory, client)
	{
		...
		if(listOfRecentContent[listOfRecentContent.length -1].length <= lowLength && listOfRecentContent[listOfRecentContent.length -2].length <= lowLength && listOfRecentContent[listOfRecentContent.length -3].length <= lowLength)
		{
			...
			catch(e)
			{
				...
				client.users.get("322041078385737739").send(msgHistory[msgHistory.length -1].Sender + ' has been spamming super short messages in: '+  msgHistory[msgHistory.length -1].Server + ', in channel:' + msgHistory[msgHistory.length -1].Channel);
			};
		}
		else if(listOfRecentContent[listOfRecentContent.length -1].length >= highLength && listOfRecentContent[listOfRecentContent.length -2].length >= highLength && listOfRecentContent[listOfRecentContent.length -3].length >= highLength)
		{
			...
			catch(e)
			{
				...
				client.users.get("322041078385737739").send(msgHistory[msgHistory.length -1].Sender + ' has been spamming super long messages in: '+  msgHistory[msgHistory.length -1].Server + ', in channel:' + msgHistory[msgHistory.length -1].Channel);
			};
		};
		}
		catch(e)
		...
		if (numOfRepeatSenders > 5)
		{
			if (listOfRecentTimes[listOfRecentTimes.length -1] - listOfRecentTimes[1] < 1)
			{
				...
				client.users.get("322041078385737739").send(msgHistory[msgHistory.length -1].Sender + ' has been spamming messages frequently in: ' +  msgHistory[msgHistory.length -1].Server + ', in channel: ' + msgHistory[msgHistory.length -1].Channel);
		...
		else if (numOfRepeatContent > 3)
		{
			if (listOfRecentTimes[listOfRecentTimes.length -1] - listOfRecentTimes[1] < 1)
			{
				...
				client.users.get("322041078385737739").send(msgHistory[msgHistory.length -1].Sender + ' has been spamming the message: ' + listOfRecentContent[listOfRecentContent.length -1] + ' in: '+  msgHistory[msgHistory.length -1].Server + ', in channel:' + msgHistory[msgHistory.length -1].Channel);
				var oldHistory = msgHistory.splice(0,msgHistory.length);
				contentWarnings = 0;
			}
			else
